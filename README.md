...
2026-08-03 一天内出现 6 次系统崩溃，分两组独立原因：

| 时间 | 类型 | 原因 |
|---|---|---|
| 12:09 / 18:55 / 21:07 | `0x139` KERNEL_SECURITY_CHECK_FAILURE (arg1=0xa 链表损坏) / `0x133` DPC_WATCHDOG | `ACE-BOOT.sys` 被装成 `Start=System`（开机常驻内核），且前一日重装 3 次 |
| 22:32 / 22:40 / 23:19 | 硬卡死无 dump | HP 诊断工具驱动反复重试 SMART 长自检，SSD 写缓存 5 分钟抖动 66 次 |

关键证据：崩溃时 ACE 的文件系统过滤驱动**未加载**（当时没在玩游戏），说明问题出在 ACE-BOOT 的常驻层，而非游戏运行时。
